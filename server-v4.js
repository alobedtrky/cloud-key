...
function previewHeaders(headers){const h={...headers};delete h['x-frame-options'];let csp=String(h['content-security-policy']||'');if(csp){csp=csp.replace(/frame-ancestors\s+'none'/i,"frame-ancestors 'self'");h['content-security-policy']=csp}h['x-frame-options']='SAMEORIGIN';return h}
function proxy(req,res){const p=http.request({hostname:'127.0.0.1',port:INNER_PORT,path:req.url,method:req.method,headers:{...req.headers,host:`127.0.0.1:${INNER_PORT}`}},r=>{res.writeHead(r.statusCode||500,previewHeaders(r.headers));r.pipe(res)});p.on('error',()=>{res.writeHead(502);res.end('Bad gateway')});req.pipe(p)}
...
server.listen(OUTER_PORT,()=>console.log(`Cloud Key V4 CMS + SEO: http://localhost:${OUTER_PORT}`));