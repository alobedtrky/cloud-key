...
for(const file of ['public/index.html','public/product.html','public/login.html','public/profile.html']){
  try{let s=fs.readFileSync(file,'utf8');if(!s.includes('/v4.css'))s=s.replace('</head>','<link rel="stylesheet" href="/v4.css"></head>');if(!s.includes('/cms-site.js'))s=s.replace('</body>','<script src="/cms-site.js"></script></body>');fs.writeFileSync(file,s)}catch{}
}
